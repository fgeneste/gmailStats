/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import ConfDetailComponent from '@/entities/conf/conf-details.vue';
import ConfClass from '@/entities/conf/conf-details.component';
import ConfService from '@/entities/conf/conf.service';
import router from '@/router';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('Conf Management Detail Component', () => {
    let wrapper: Wrapper<ConfClass>;
    let comp: ConfClass;
    let confServiceStub: SinonStubbedInstance<ConfService>;

    beforeEach(() => {
      confServiceStub = sinon.createStubInstance<ConfService>(ConfService);

      wrapper = shallowMount<ConfClass>(ConfDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { confService: () => confServiceStub },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundConf = { id: 123 };
        confServiceStub.find.resolves(foundConf);

        // WHEN
        comp.retrieveConf(123);
        await comp.$nextTick();

        // THEN
        expect(comp.conf).toBe(foundConf);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundConf = { id: 123 };
        confServiceStub.find.resolves(foundConf);

        // WHEN
        comp.beforeRouteEnter({ params: { confId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.conf).toBe(foundConf);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
